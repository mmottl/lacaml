...
static inline void STR(NAME, _range)(integer N, NUMBER *A_data, NUMBER *B_data,
                                     NUMBER *C_data) {
  for (int i = 0; i < N; i++) {
    NUMBER a = A_data[i];
    NUMBER b = B_data[i];
...
CAMLprim value NAME(value vPKIND, intnat vPINIT, intnat vM, intnat vN,
                    ...
                    value vB, intnat vCR, intnat vCC, value vC) {
  CAMLparam3(vA, vB, vC);
...
CAMLprim value BC_NAME(value *argv, int __unused argn) {
  return NAME(argv[0], Int_val(argv[1]), Int_val(argv[2]), Int_val(argv[3]),
              ...
              Int_val(argv[8]), argv[9], Int_val(argv[10]), Int_val(argv[11]),
              argv[12]);
}
