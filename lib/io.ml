...
module Context = struct
  ...
  let create n = if n < 1 then failwith "Lacaml.Io.Context.create: n < 1" else n
...
let pp_mat_gen ?(pp_open = pp_open) ?(pp_close = pp_close) ?pp_head ?pp_foot
    ?(pp_end_row = pp_end_row_newline) ?(pp_end_col = pp_end_col_space) ?pp_left
    ?pp_right ?(pad = some_space) ?(ellipsis = !Context.ellipsis_default)
    ?(vertical_context = !Context.vertical_default)
    ?(horizontal_context = !Context.horizontal_default) pp_el ppf mat =
    ...
    if n > 0 then (
      ...
      let do_pp_right =
        ...
        | Some pp_right ->
            ...
            fun ppf row ->
              let str, n_str =
                pp_buf (fun ppf -> pp_right ppf row) buf buf_ppf
              in
              if n_str <> 0 then (
      ...
      | Some pad_c ->
          ...
          if pp_head <> None then (
            fmt_row_label ~ellipsis:head_foot_ellipsis ~src_r:0 heads head_label;
            do_pp_right ppf 0;
            pp_end_row ppf 0);
          ...
          if has_ver then (
            ...
            fmt_row_label ~src_r:v1
              (Array.make disp_n ellipsis)
          ...
          if pp_foot <> None then (
            pp_end_row ppf m;
            let m1 = m + 1 in
            fmt_row_label ~ellipsis:head_foot_ellipsis ~src_r:m1 foots
              foot_label;
            do_pp_right ppf m1)
      | None -> (
...
let pp_labeled_mat_gen pp_el ?pp_head ?pp_foot ?pp_left ?pp_right ?pad ?ellipsis
  ...
  pp_mat_gen ?pp_head ?pp_foot ?pp_left ?pp_right ?pad ?ellipsis
    ?vertical_context ?horizontal_context pp_el ppf mat
...
let pp_lmat_gen (pp_labeled_mat : ('el, 'elt) pp_labeled_mat)
    ?(print_head = true) ?(print_foot = true) ?(print_left = true)
    ?(print_right = true) ?row_labels ?col_labels ?pad ?ellipsis
    ?vertical_context ?horizontal_context () ppf mat =
  let pp_head, pp_foot =
    match col_labels with
    | Some col_labels when print_head ->
        if Array.length col_labels <> Array2.dim2 mat then
          invalid_arg "Io.pp_lmat_gen: dim(col_labels) <> dim2(mat)";
  ...
  let pp_left, pp_right =
    ...
    | Some row_labels ->
        ...
        if Array.length row_labels <> m then
          invalid_arg "Io.pp_lmat_gen: dim(row_labels) <> dim1(mat)";
...
let pp_labeled_vec_gen pp_el ?pp_head ?pp_foot ?pp_left ?pp_right ?pad ?ellipsis
  ...
  let pp_left =
    ...
    | None -> get_some_pp_left_right m
    | Some None -> None
    | Some (Some pp_left) -> Some pp_left
  in
...
let pp_labeled_rvec_gen pp_el ?pp_head:this_pp_head ?pp_foot:this_pp_foot
    ?pp_left:this_pp_left ?pp_right:this_pp_right ?pad ?ellipsis
    ?vertical_context ?horizontal_context () ppf vec =
  let pp_head =
    match this_pp_left with
    | None -> some_pp_print_int
    | Some None -> None
    | Some (Some this_pp_left) -> Some this_pp_left
  in
...
let get_lvec_name = function None -> None | Some name -> Some [| name |]

let pp_lvec_gen (pp_lmat : ('el, 'elt) pp_lmat) ?print_head ?print_foot
    ?print_left ?(print_right = false) ?labels:row_labels ?name ?pad ?ellipsis
    ?vertical_context ?horizontal_context () ppf vec =
  let mat = from_col_vec vec in
  let col_labels = get_lvec_name name in
  pp_lmat ?print_head ?print_foot ?print_left ~print_right ?row_labels
    ?col_labels ?pad ?ellipsis ?vertical_context ?horizontal_context () ppf mat
...
let pp_rlvec_gen (pp_lmat : ('el, 'elt) pp_lmat) ?print_head:print_left
    ?print_foot:print_right ?print_left:print_head ?print_right:this_print_right
    ?labels:col_labels ?name ?pad ?ellipsis ?vertical_context
    ?horizontal_context () ppf vec =
  let mat = from_row_vec vec in
  let row_labels = get_lvec_name name in
  let print_foot =
    match this_print_right with
    | None -> false
    | Some this_print_right -> this_print_right
  in
  pp_lmat ?print_head ~print_foot ?print_left ?print_right ?row_labels
    ?col_labels ?pad ?ellipsis ?vertical_context ?horizontal_context () ppf mat
...
let pp_ovec ppf pp_el vec =
  if Array1.dim vec = 0 then pp_print_string ppf "[||]"
    ...
    pp_mat_gen ~pp_open:pp_ocaml_open_vec ~pp_close:pp_ocaml_close_vec
      ~pp_end_row:pp_end_row_semi pp_el ppf (from_col_vec vec)
...
let pp_rovec ppf pp_el vec =
  if Array1.dim vec = 0 then pp_print_string ppf "[||]"
    ...
    pp_mat_gen ~pp_open:pp_ocaml_open_rvec ~pp_close:pp_ocaml_close_rvec
      ~pp_end_row:pp_end_row_semi_space ~pad:None pp_el ppf (from_col_vec vec)
...
let pp_omat ppf pp_el mat =
  if Array2.dim1 mat = 0 || Array2.dim2 mat = 0 then pp_print_string ppf "[||]"
    ...
    pp_mat_gen ~pp_open:pp_ocaml_open_mat ~pp_close:pp_ocaml_close_mat
      ~pp_end_row:pp_ocaml_end_row_mat ~pp_end_col:pp_end_col_semi_space pp_el
      ppf mat
