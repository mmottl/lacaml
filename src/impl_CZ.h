...
CAMLprim value LFUN(dotu_stub)(intnat vN, intnat vOFSX, intnat vINCX, value vX,
  ...
  res = DOTU(&N, X_data, &INCX, Y_data, &INCY);
...
CAMLprim value LFUN(dotu_stub_bc)(value *argv, int __unused argn) {
  return LFUN(dotu_stub)(Int_val(argv[0]), Int_val(argv[1]), Int_val(argv[2]),
...
CAMLprim value LFUN(dotc_stub)(intnat vN, intnat vOFSX, intnat vINCX, value vX,
                               intnat vOFSY, intnat vINCY, value vY) {
  CAMLparam2(vX, vY);
  ...
  caml_enter_blocking_section(); /* Allow other threads */
  res = DOTC(&N, X_data, &INCX, Y_data, &INCY);
  caml_leave_blocking_section(); /* Disallow other threads */
...
CAMLprim value LFUN(dotc_stub_bc)(value *argv, int __unused argn) {
  return LFUN(dotc_stub)(Int_val(argv[0]), Int_val(argv[1]), Int_val(argv[2]),
                         argv[3], Int_val(argv[4]), Int_val(argv[5]), argv[6]);
}
...
extern REAL FUN(lansy)(char *NORM, char *UPLO, integer *N, NUMBER *A,
...
CAMLprim value LFUN(gecon_stub)(intnat vN, intnat vAR, intnat vAC, value vA,
  ...
  caml_enter_blocking_section(); /* Allow other threads */
  FUN(gecon)(&NORM, &N, A_data, &rows_A, &ANORM, &RCOND, WORK_data, RWORK_data,
             &INFO);
  caml_leave_blocking_section(); /* Disallow other threads */
...
extern void FUN(sycon)(char *UPLO, integer *N, COMPLEX *A, integer *LDA,
...
extern void FUN(pocon)(char *UPLO, integer *N, COMPLEX *A, integer *LDA,
                       REAL *ANORM, REAL *RCOND, COMPLEX *WORK, REAL *RWORK,
                       integer *INFO);

CAMLprim value LFUN(pocon_stub)(value vUPLO, intnat vN, intnat vAR, intnat vAC,
                                value vA, value vWORK, value vRWORK,
                                double vANORM) {
  CAMLparam3(vA, vWORK, vRWORK);
  CAMLlocal1(v_rcond);
  ...
  caml_enter_blocking_section(); /* Allow other threads */
  FUN(pocon)(&UPLO, &N, A_data, &rows_A, &ANORM, &RCOND, WORK_data, RWORK_data,
             &INFO);
  caml_leave_blocking_section(); /* Disallow other threads */
...
extern void FUN(gees)(char *JOBVS, char *SORT, LAPACK_CZ_SELECT1 SELECT,
                      integer *N, COMPLEX *A, integer *LDA, integer *SDIM,
                      COMPLEX *W, COMPLEX *VS, integer *LDVS, COMPLEX *WORK,
                      integer *LWORK, REAL *RWORK, integer *BWORK,
                      integer *INFO);

CAMLprim value LFUN(gees_stub)(value vJOBVS, value vSORT, intnat vSELECT,
                               value vSELECT_FUN, intnat vN, intnat vAR,
                               intnat vAC, value vA, value vW, intnat vVSR,
                               intnat vVSC, value vVS, value vWORK,
                               intnat vLWORK, value vRWORK, value vBWORK) {
  ...
  FUN(gees)(&JOBVS, &SORT, select_function, &N, A_data, &rows_A, &SDIM, W_data,
            VS_data, &rows_VS, WORK_data, &LWORK, RWORK_data, BWORK_data,
            &INFO);
...
CAMLprim value LFUN(gees_stub_bc)(value *argv, int __unused argn) {
  return LFUN(gees_stub)(argv[0], argv[1], Int_val(argv[2]), argv[3],
                         ...
                         argv[7], argv[8], Int_val(argv[9]), Int_val(argv[10]),
                         argv[11], argv[12], Int_val(argv[13]), argv[14],
                         argv[15]);
}
...
extern void FUN(gesvd)(char *JOBU, char *JOBVT, integer *M, integer *N,
...
CAMLprim intnat LFUN(gesvd_stub)(value vJOBU, value vJOBVT, intnat vM,
  ...
  FUN(gesvd)(&JOBU, &JOBVT, &M, &N, A_data, &rows_A, S_data, U_data, &rows_U,
             VT_data, &rows_VT, WORK_data, &LWORK, RWORK_data, &INFO);
  caml_leave_blocking_section(); /* Disallow other threads */
...
extern void FUN(geev)(char *JOBVL, char *JOBVR, integer *N, COMPLEX *A,
...
CAMLprim intnat LFUN(geev_stub)(intnat vAR, intnat vAC, value vA, intnat vN,
                                intnat vOFSW, value vW, intnat vVLR,
                                intnat vVLC, value vVL, value vJOBVL,
                                intnat vVRR, intnat vVRC, value vVR,
                                value vJOBVR, value vWORK, intnat vLWORK,
                                value vRWORK) {
  ...
  caml_enter_blocking_section(); /* Allow other threads */
  FUN(geev)(&JOBVL, &JOBVR, &N, A_data, &rows_A, W_data, VL_data, &rows_VL,
            VR_data, &rows_VR, WORK_data, &LWORK, RWORK_data, &INFO);
  caml_leave_blocking_section(); /* Disallow other threads */
...
CAMLprim value LFUN(geev_stub_bc)(value *argv, int __unused argn) {
  return Val_int(LFUN(geev_stub)(
      ...
      argv[14], Int_val(argv[15]), argv[16]));
}
