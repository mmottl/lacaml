(* File: io.mli
   ...
   Jane Street Holding, LLC <opensource@janestreet.com>
...
module Context : sig
  ...
  (* [set_dim_defaults def] sets both vertical and horizontal context default to
     [def]. *)
end
...
(** [pp_mat_gen
    ...
    [pp_open ppf] is called whenever printing of a matrix [mat] is started,
    [pp_close ppf] whenever printing is complete. These functions are not called
    when the matrix is empty.

    [pp_head other_ppf col] is used to print a header for column [col] in matrix
    [mat]. This header is right-aligned and eventually padded using
    [Some pad]-character to match the matrix rows in the column beneath. The
    passed formatter [other_ppf] is not identical to [ppf]!

    [pp_foot other_ppf col] is used to print a footer for column [col] in matrix
    [mat]. It is similar to [pp_head col other_ppf] otherwise.

    [pp_end_row ppf row] is called on row number [row] and formatter [ppf]
    whenever the end of a row has been reached.

    [pp_end_col ppf ~row ~col] is called on the row number [row], column number
    [col] and formatter [ppf] whenever the element at this position has been
    printed and if it is not the last element in the row.

    [pp_left ppf row] is called on row number [row] and formatter [ppf] to print
    labels to the left of each row. The labels are right-aligned within a
    virtual column.

    [pp_right ppf row] is called on row number [row] and formatter [ppf] to
    print labels to the right of each row. The labels are left-aligned.

    The character [pad] is used to pad matrix elements for right-aligning them
    appropriately. If it is set to [None], no alignment will be performed.

    [ellipsis] is used as a filler when elements need to be skipped in the case
    of printing with contexts.

    [vertical_context] determines the number of initial and final rows to be
    printed. Intermediate row will be skipped, and one row containing ellipsis
    elements will be printed in their place instead. [None] chooses no context,
    [Some v] sets the vertical context to [v].

    [horizontal_context] determines the number of initial and final columns to
    be printed. Intermediate columns will be skipped, and one columns containing
    ellipsis elements will be printed in their place instead. [None] chooses no
    context, [Some h] sets the horizontal context to [h].

    [pp_el other_ppf el] is called on formatter [other_ppf] (not [ppf]!) and
    each matrix element.
...
(** [pp_vec ppf vec] prints a vector [vec] to formatter [ppf] using the
    defaults. *)
...
(** [pp_labeled_vec ?pp_head ?pp_foot ?pp_left ?pp_right ?pad
    ...
    @param pp_left
      default = [Some pp_int32_el] for vector rows/cols (= not in header/footer
      row/col)
...
(** [pp_lvec ?print_head ?print_foot ?print_left ?print_right
    ...
    It is the duty of the user to make sure that the array containing the labels
    is sufficiently large for the given vector.
...
(** [pp_lmat ?print_head ?print_foot ?print_left ?print_right
    ...
    It is the duty of the user to make sure that the arrays containing the row-
    and column labels are sufficiently large for the given matrix.
...
type ('el, 'elt) pp_el_ovec =
  ...
  ('el, 'elt, fortran_layout) Array1.t ->
  unit
(** [pp_el_ovec ppf pp_el vec] prints the vector [vec] to formatter [ppf] in
    OCaml-style using the element printer [pp_el]. *)

val pp_ovec : ('el, 'elt) pp_el_ovec
(** [pp_ovec ppf pp_el vec] prints the column vector [vec] to formatter [ppf] in
    OCaml-style using the element printer [pp_el]. *)

val pp_rovec : ('el, 'elt) pp_el_ovec
(** [pp_rovec ppf pp_el vec] prints the row vector [vec] to formatter [ppf] in
    OCaml-style using the element printer [pp_el]. *)
...
val pp_omat :
  ...
  unit
(** [pp_omat ppf pp_el mat] prints matrix [mat] to formatter [ppf] in
    OCaml-style using the element printer [pp_el]. *)
...
module Toplevel : sig
  val lsc : int -> unit
  (* Shortcut for setting both the horizontal and vertical context. Remember as
     "Lacaml set contexts". This function is automatically bound as [lsc] in
     toplevels for quick access. *)
